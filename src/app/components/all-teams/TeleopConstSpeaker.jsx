...
const AllTeamsTeleop = () => {
    ...
    const finalData = Object.values(teamData).map(item => ({
      ...item,
      avgSpeakerNotes: Math.round(item.totalSpeakerNotes / item.count),
    //   avgAmpNotes: item.totalAmpNotes / item.count,
  }));

  const sortedData = finalData.sort((a, b) => (b.avgSpeakerNotes) - (a.avgSpeakerNotes));

  const maxYValue = Math.max(...sortedData.map(item => item.avgSpeakerNotes));
  ...
  return (
      <ResponsiveContainer width="100%" height="100%">
          <BarChart data={sortedData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
              ...
              <Bar name="Speaker Notes" dataKey="avgSpeakerNotes" stackId="a" fill="#3b82f6" />
